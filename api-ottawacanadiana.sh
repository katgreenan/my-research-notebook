...
pages=$(curl 'http://search.canadiana.ca/search/?df=1800&q=ottawa&dt=1900&field=&fmt=json' | jq '.pages')
...
do
        curl 'http://search.canadiana.ca/search/'${i}'?df=1800&q=ottawa&dt=1900&field=&fmt=json' | jq '.docs[] | {key}' >> results.txt
done
